...
public:
    ...
    void CreateBloodParticles(const sf::Vector2f& position, float reflectAngle, int m_minBloodParticles, int m_maxBloodParticles, float maxBloodSpeed, float bloodParticleLifetime)
    {
        ...
        for (int i = 0; i < numParticles; ++i)
        {
            float velocityMagnitude = (static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX)) * maxBloodSpeed; // Случайная величина скорости

            sf::RectangleShape particle; // Создаем круговую частицу
            particle.setSize(sf::Vector2f(rand() % 12, rand() % 6));
            particle.setPosition(position);
            particle.setTexture(PARTICLE.Give_Me_Random_Blood_Texture()); // текстурa для частиц крови
